...
def test_toml_functionality():
    ...
    test_toml = 'research_model = "gpt-4o-mini"\nenable_clarification = true'
...
def test_config_with_environment_variables():
    ...
    env_vars = [
        "RESEARCH_MODEL", "ENABLE_CLARIFICATION", "TRIAGE_MODEL", "CLARIFIER_MODEL"
    ]
    original_values = {}
    for var in env_vars:
...
def test_toml_vs_legacy_format():
    ...
    assert str(toml_style["enable_clarification"]).lower() == legacy_style["ENABLE_CLARIFICATION"]
...
def test_config_defaults():
    ...
    env_vars = [
        "RESEARCH_MODEL", "ENABLE_CLARIFICATION", "TRIAGE_MODEL", "CLARIFIER_MODEL",
        "RESEARCH_TIMEOUT", "POLL_INTERVAL", "MAX_RETRIES"
    ]
    original_values = {}
    for var in env_vars:
        original_values[var] = os.environ.get(var)
    ...
    try:
        ...
        assert config.triage_model == "gpt-4o-mini"  # Default
        assert config.clarifier_model == "gpt-4o-mini"  # Default
