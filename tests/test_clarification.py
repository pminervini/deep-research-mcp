...
Uses real OpenAI API calls with gpt-4o-mini to keep costs low.
"""
...
def test_triage_agent_specific_query(config):
    ...
    result = agent.analyze_query("What is the molecular structure of caffeine and its chemical formula?")
...
def test_triage_agent_error_handling(config):
    ...
    try:
        ...
        assert "error" in result["reasoning"].lower() or "could not parse" in result["reasoning"].lower()
...
def test_clarifier_agent(config):
    ...
    qa_pairs = [
        {"question": "Are you interested in hardware or software aspects?", "answer": "Hardware"},
        {"question": "What time period should we focus on?", "answer": "Last 5 years"},
        {"question": "Are you looking for research papers or commercial applications?", "answer": "Commercial applications"}
    ]
...
def test_clarification_manager_workflow(config):
    ...
    if result.get("needs_clarification", False):
        ...
        answers = ["Neural networks" if i == 0 else f"Answer {i+1}" for i in range(len(questions))]
