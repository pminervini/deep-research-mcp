...
USAGE EXAMPLES:
  ...
  uv run deep-research-cli research "What are the latest treatments for diabetes?"

  # Research with a specific provider and model
  uv run deep-research-cli --provider gemini research "Healthcare costs in the US"

  # Research with a custom system prompt from file
  uv run deep-research-cli research "AI trends" --system-prompt-file prompts/custom.txt

  # Research via MCP server (client mode)
  uv run deep-research-cli research "Climate change" --server-url http://localhost:8080/mcp

  # Check task status
  uv run deep-research-cli status abc123-def456
