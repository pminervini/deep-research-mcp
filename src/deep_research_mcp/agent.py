    ...
    def __init__(self, config: ResearchConfig):
        ...
        if self.config.provider in {"openai"}:
            ...
            raise ConfigurationError(
                f"Provider '{self.config.provider}' is not supported"
            )
...
Additionally, if after some searching you find out that you need more information to answer the question, you can use `final_answer` with your request for clarification as argument to request for more information."""
    ...
    async def research(
        ...
        if self.config.provider in {"openai"}:
            ...
            if self.config.api_style == "chat_completions":
                result = await self._run_chat_completions_research(
                ...
                try:
                    response = await self._create_openai_research_task(
                        input_messages, tools
                    )
    ...
    async def _run_gemini_research(
        ...
        if include_code_interpreter:
            self.logger.debug(
                "Gemini Deep Research does not expose a separate code_execution toggle; ignoring include_code_interpreter"
            )
        ...
        while time.time() - start_time < self.config.timeout:
            try:
                ...
                if interaction.status == "requires_action":
                    raise ResearchError(
                        f"Research task {task_id} requires client tool handling, which this integration does not support"
                    )
                if interaction.status in {"failed", "cancelled", "incomplete"}:
                    raise ResearchError(
                        self._extract_gemini_failure_message(interaction)
                    )
